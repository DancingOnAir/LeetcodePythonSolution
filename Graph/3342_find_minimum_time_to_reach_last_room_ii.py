    ...
    def minTimeToReach(self, moveTime: List[List[int]]) -> int:
        ...
        while True:
            ...
            for x, y in (i + 1, j), (i - 1, j), (i, j + 1), (i, j - 1):
                if 0 <= x < m and 0 <= y < n:
                    new_dist = max(d, moveTime[x][y]) + (i + j) % 2 + 1
...
def test_min_time_to_reach():
    solution = Solution()
    assert solution.minTimeToReach([[0, 4], [4, 4]]) == 7, 'wrong result'
    assert solution.minTimeToReach([[0, 0, 0, 0], [0, 0, 0, 0]]) == 6, 'wrong result'
    assert solution.minTimeToReach([[0, 1], [1, 2]]) == 4, 'wrong result'
